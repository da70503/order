...
// 🎲 種子型亂數（決定論 PRNG）：_seededFloat 仍保留供簽章等用途；掉落／娃娃已改即時 Math.random()。
...
// 🎲 強化成敗、掉落抽取、娃娃開袋等＝即時 Math.random()（每次嘗試獨立擲骰，可 save/load 重抽）。
//    player.enSeed／lootSeq／dollSeq 仍可能留在舊存檔中，但已不再參與擲骰結果。

// 🎲 掉落／製作／兌換／潘朵拉／開箱／裂痕領取／碧恩賦予 等共用即時亂數（tag 保留供除錯，不影響結果）。
...
// 一般模式：v2.6.40 分段放大制（EXP_T + 50~99 倍率放大）；經典模式：天堂經典經驗表（EXP_REQ_CLASSIC）。
...
];   // index＝等級·天堂經典表（經典模式升級需求）
function _expReqClassic(lv) {
    if (lv >= 100) return Infinity;
    return EXP_REQ_CLASSIC[lv] || Infinity;
}
// 一般模式：v2.6.40 分段放大制（50~69×2、70~74×4 … 90~99×1024；49以下用 EXP_T）
...
function getExpReq(lv, classicMode) {
    if (classicMode === undefined) classicMode = !!(player && player.classicMode);
    return classicMode ? _expReqClassic(lv) : _expReqOldV1(lv);
}
