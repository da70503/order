...
function killMob(idx) {
    ...
    player.exp += Math.floor(mob.exp * getExpGainMult(player.lv) * (1 + dollFieldVal('expBonus') / 100));
    checkLvUp();
    // 🤝 協力傭兵經驗：每名非倒地傭兵各得 MERC_EXP_SHARE（不減玩家）
    if (player.allies && player.allies.length && mob.exp) {
        player.allies.forEach(a => {
            if (!a || a._downed) return;
            let _gain = Math.floor(mob.exp * getExpGainMult(a.lv || 1) * MERC_EXP_SHARE);
            if (_gain <= 0) return;
            a.exp = (a.exp || 0) + _gain;
