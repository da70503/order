...
function auditTrackKill(mob) {
    if (!mob || typeof playerKillExpGain !== 'function') return;
    let g = playerKillExpGain(mob);   // 與 killMob 隊長實得一致（經典均分／一般全額）
    if (g > 0) _audit.exp += g;
    _audit.kills++;
}
...
//   經典模式現存差異：天堂經驗表、組隊經驗均分、死亡損失 5% 經驗、隱藏祝福/精通/席琳、停用武器/盾/騎士特效。
function classicDropMult() { return 1; }
function trialItemDropMult(id) { return 1; }
// 🎮 經典模式：天堂組隊經驗（先加成再均分；一般模式不啟用）
function partyExpShareCount() { return 1 + ((player.allies || []).filter(a => a && !a._downed).length); }
function partyExpBonusPct() {
    if (!player || !player.classicMode) return 0;
    let _mates = (player.allies || []).filter(a => a && !a._downed).length;
    if (_mates <= 0) return 0;
    return _mates * (player.cls === 'royal' ? 8 : 4);   // 👑 王族隊長每隊友 +8%；其餘 +4%
}
function classicExpSharePool(mob) {
    return (mob.exp || 0) * (1 + partyExpBonusPct() / 100) / partyExpShareCount();
}
function playerKillExpGain(mob) {
    if (!mob || typeof getExpGainMult !== 'function') return 0;
    let base = player.classicMode ? classicExpSharePool(mob) : (mob.exp || 0);
    return Math.floor(base * getExpGainMult(player.lv) * (1 + dollFieldVal('expBonus') / 100));
}
function mercKillExpGain(mob, ally) {
    if (!mob || typeof getExpGainMult !== 'function') return 0;
    if (player.classicMode) return Math.floor(classicExpSharePool(mob) * getExpGainMult(ally.lv || 1));
    return Math.floor((mob.exp || 0) * getExpGainMult(ally.lv || 1) * MERC_EXP_SHARE);
}
function killMob(idx) {
    ...
    { let _pGain = playerKillExpGain(mob); if (_pGain > 0) player.exp += _pGain; }   // 一般：隊長全額；經典：均分後一份
    checkLvUp();
    // 🤝 協力傭兵：一般＝各得 MERC_EXP_SHARE（不減隊長）；經典＝與隊長同額均分份
    if (player.allies && player.allies.length && mob.exp) {
        player.allies.forEach(a => {
            if (!a || a._downed) return;
            let _gain = mercKillExpGain(mob, a);
            if (_gain <= 0) return;
            a.exp = (a.exp || 0) + _gain;
            a._expGained = (a._expGained || 0) + _gain;
            let _up = 0;
            while ((a.lv || 1) < 100 && a.exp >= getExpReq(a.lv, a.classicMode)) { a.exp -= getExpReq(a.lv, a.classicMode); a.lv++; if (a.lv >= 50) a.bonus = (a.bonus || 0) + 1; _up++; }   // 比照 checkLvUp 升級曲線（傭兵依自身經典旗標選表）
