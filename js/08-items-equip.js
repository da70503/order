function gainItem(id, cnt=1, silent=false, forceNormal=false, affixOld=false) {
    ...
    try { if (!ffSuppressRareLootVfx() && _vfxLootCtx && d && d.gachaWeight === 1 && typeof vfxRareDrop === 'function') vfxRareDrop(d.n); } catch(e){}   // ✨ VFX：潘朵拉權重=1 稀有閃光（長時間補跑/離線結算略過·與「掛機期間獲得」門檻一致；短補跑<3秒仍播）
...
function rollAttrAffix() {
    ...
    return (ATTR_ELE_PREFIX[ele] || 'fr') + tier;
}
...
function getItemFullName(item) {
    ...
    let en = (item.en > 0) ? (`+${capEn(item.en, d)} `) : ((Number(item.en) || 0) < 0 ? `${item.en} ` : "");   // 🔧 一律顯示 +N（夾擠至上限：武器+20/防具+15/飾品+5）；🏰 詛咒降階的負值(如 -1)原樣顯示
...
function doEnhance(targetUid, isEq = true) {
    ...
    let _cap = enhanceCap(d);   // 🔧 強化上限：武器+20 / 防具+15 / 飾品+5
    ...
    let success = false, destroy = false;
    let safe = d.safe || 0;
