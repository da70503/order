...
function _vfxClearAll() {   // 🎚️ v3.0.73 換地圖/回城即清空狩獵區殘留特效（死亡殘影 vfx-ghost／法術 vfx-spell／冰凍／怪技能特效）→上一張地圖尚在播放的死亡動畫不會殘留、蓋到村莊或新地圖介面。冰凍/怪技能追蹤 dict 一併清鍵避免孤兒元素。
    ...
    try { if (typeof _mobAnimCacheTrimForMap === 'function') _mobAnimCacheTrimForMap(); } catch (e) {}   // 🚀 換圖：只釋放「非本地圖池」快取·保留回獵區流暢
    try { _mobRenderCache = null; } catch (e) {}
}
...
if (typeof castSkill === 'function' && !castSkill._vfxWrapped) {
    ...
    castSkill = function (skId) {
        ...
        if (r) {
            try {
                let _chain = _skillUsesChainAttack(sk, skId);
                if (_chain > 0 && typeof _playerMorphTriggerChainAttack === 'function') _playerMorphTriggerChainAttack(_chain);
                else if (typeof _playerMorphTrigger === 'function') _playerMorphTrigger('skill', skId);
            } catch (e) {}
        }   // 🧝 v3.0.46 玩家戰鬥 sprite：僅施法成功才播 skill（屠宰者/三重矢改連播攻擊動畫）
...
const MOB_ANIM_CACHE_MAX = 64;   // 🚀 已載入動畫怪硬上限（LRU·釘選場上怪＋本地圖池）；防無限成長·單圖掛機通常遠低於此
let _mobAnimCacheLru = [];         // 怪名 LRU 序（舊→新）·僅在 probe／新怪登場時更新·避免每幀 touch 造成不必要驅逐
function _mobAnimMapPoolNames(mapKey) {   // 該地圖出怪池內的動畫怪名（換圖時優先保留·回村仍留 lastBattleMap 池）
    let s = new Set();
    if (!mapKey || typeof DB === 'undefined' || !DB.mobs) return s;
    try {
        if (typeof KING_ROOMS !== 'undefined' && KING_ROOMS[mapKey]) {
            let kr = KING_ROOMS[mapKey];
            if (kr.boss) { let b = DB.mobs[kr.boss]; if (b && b.n && MOB_ANIM_NAMES.has(b.n)) s.add(b.n); }
            if (kr.minion) { let m = DB.mobs[kr.minion]; if (m && m.n && MOB_ANIM_NAMES.has(m.n)) s.add(m.n); }
            if (kr.bosses) kr.bosses.forEach(id => { let b = DB.mobs[id]; if (b && b.n && MOB_ANIM_NAMES.has(b.n)) s.add(b.n); });
            return s;
        }
        let pool = DB.maps && DB.maps[mapKey];
        if (!pool) return s;
        pool.forEach(id => { let d = DB.mobs[id]; if (d && d.n && MOB_ANIM_NAMES.has(d.n)) s.add(d.n); });
    } catch (e) {}
    return s;
}
function _mobAnimPinnedNames() {   // 場上仍存在的動畫怪不驅逐（避免播到一半失幀）
    let s = new Set();
    try {
        if (typeof mapState !== 'undefined' && mapState && mapState.mobs) {
            mapState.mobs.forEach(m => { if (m && m.n && MOB_ANIM_NAMES.has(m.n)) s.add(m.n); });
        }
    } catch (e) {}
    return s;
}
function _mobAnimCacheKeepNames() {   // 釘選＋當前／上次狩獵地圖池→盡量保留·減少回圖重載
    let keep = _mobAnimPinnedNames();
    try {
        if (typeof mapState !== 'undefined' && mapState && mapState.current) _mobAnimMapPoolNames(mapState.current).forEach(n => keep.add(n));
        if (typeof player !== 'undefined' && player && player.lastBattleMap) _mobAnimMapPoolNames(player.lastBattleMap).forEach(n => keep.add(n));
    } catch (e) {}
    return keep;
}
function _mobAnimCacheReleaseImages(node) {   // 清空 Image.src 助 GC 釋放解碼後點陣
    if (!node || typeof node !== 'object') return;
    if (Array.isArray(node)) {
        for (let im of node) { try { if (im) { im.onload = im.onerror = null; im.src = ''; } } catch (e) {} }
        return;
    }
    for (let k in node) _mobAnimCacheReleaseImages(node[k]);
}
function _mobAnimCacheTouch(name) {
    if (!name) return;
    let i = _mobAnimCacheLru.indexOf(name);
    if (i >= 0) _mobAnimCacheLru.splice(i, 1);
    _mobAnimCacheLru.push(name);
}
function _mobAnimCacheEvictOne(skip) {
    for (let i = 0; i < _mobAnimCacheLru.length; i++) {
        let name = _mobAnimCacheLru[i];
        if (skip.has(name)) continue;
        if (_mobAnimCache[name] === 'probing') continue;
        _mobAnimCacheReleaseImages(_mobAnimCache[name]);
        delete _mobAnimCache[name];
        _mobAnimCacheLru.splice(i, 1);
        try { if (typeof _deathFxCache !== 'undefined' && _deathFxCache[name]) delete _deathFxCache[name]; } catch (e) {}
        return true;
    }
    return false;
}
function _mobAnimCacheTrim(maxEntries) {   // 僅在超過硬上限時逐筆驅逐
    maxEntries = maxEntries || MOB_ANIM_CACHE_MAX;
    if (_mobAnimCacheLru.length <= maxEntries) return;
    let keep = _mobAnimCacheKeepNames();
    while (_mobAnimCacheLru.length > maxEntries) {
        if (!_mobAnimCacheEvictOne(keep)) break;
    }
}
function _mobAnimCacheTrimForMap() {   // 換圖：先清「不在保留集」的舊圖怪·再套用硬上限
    let keep = _mobAnimCacheKeepNames();
    for (let i = _mobAnimCacheLru.length - 1; i >= 0; i--) {
        let name = _mobAnimCacheLru[i];
        if (keep.has(name) || _mobAnimCache[name] === 'probing') continue;
        _mobAnimCacheReleaseImages(_mobAnimCache[name]);
        delete _mobAnimCache[name];
        _mobAnimCacheLru.splice(i, 1);
        try { if (typeof _deathFxCache !== 'undefined' && _deathFxCache[name]) delete _deathFxCache[name]; } catch (e) {}
...
function _mobAnimProbe(name) {
    ...
    let finish = () => {
        if (--pending > 0) return;
        _mobAnimCache[name] = (out.idle || out.spawn || out.attack || out.skill || out.hurt || out.death) ? out : null;
        _mobAnimCacheTouch(name);
        _mobAnimCacheTrim();
    };
...
function _mobAnimApply() {
    ...
    for (let c of cards) {
        ...
        if (!m._animSpawned) { m._animSpawned = true; _mobAnimCacheTouch(m.n); if (a.spawn) _mobAnimTrigger(m, 'spawn'); }   // 🎬 登場：該怪物物件首次被動畫系統看到→播登場一輪（每隻一次·順便 touch LRU）
...
let _pmState = { act: null, t: 0, prevHp: null, name: null, el: null, imgs: null, chainAtk: 0 };
...
function _skillUsesChainAttack(sk, skId) {
    if (!sk) return 0;
    if (sk.slaughter || skId === 'sk_elf_triple') return sk.hits || 3;
    return 0;
}
function _playerMorphTriggerChainAttack(n) {   // ⚔️ 多段物理攻擊技（屠宰者/三重矢）：連播 n 次攻擊動畫，不播施法動作
    if (!(n > 0) || !_playerBattleForm()) return;
    let st = _pmState;
    if (st.act === 'death') return;
    st.chainAtk = Math.max(0, n - 1);
    st.pendAtk = false;
    st.act = 'attack';
    st.t = Date.now();
}
function _playerMorphTrigger(k, skId) {   // js/04 attack／castSkill·manualCast 包裝 skill／HP-delta hurt 呼叫（🗡️ v3.0.67 職業形態亦適用·呼叫端零改動）
    ...
    if (st.act === 'skill' && (k === 'hurt' || k === 'attack')) {   // 🔮 v3.0.94 施法鎖定：技能動畫優先——受擊不打斷；攻擊排隊(pendAtk)到技能播完才播（原本攻擊會立刻蓋掉技能動畫）
        let a = _morphBattleCache[form.key];
        let seq = (a && a !== 'probing') ? ((!st.skGen && a.wskill) ? a.wskill : a.skill) : null;   // 取「實際播放中」的序列（武器專屬 wskill 優先·與 _playerMorphApply 一致）
        if (seq && (Date.now() - st.t) < seq.length * _skillFrameMs(seq.length)) { if (k === 'attack') st.pendAtk = true; return; }
    }
    if (k === 'skill') st.skGen = (skId === 'sk_warrior_roar');   // 🗡️ v3.0.70 戰士咆哮用「通用」skill 動作（CSV 規則）·其餘技能優先武器專屬 wskill
    if (k === 'attack') st.chainAtk = 0;
...
function _playerMorphRemove() {
    ...
    _pmState.el = null; _pmState.imgs = null; _pmState.act = null; _pmState.name = null; _pmState.prevHp = null; _pmState.pendAtk = false; _pmState.chainAtk = 0;
}
    ...
    if (_pmState.act) {
        ...
        if (seq) {
            ...
            if (_pmState.act === 'death') { act = 'death'; f = Math.min(ff, seq.length - 1); }
                ...
                let _wasSkill = (_pmState.act === 'skill');
                let _wasAttack = (_pmState.act === 'attack');
                if (_wasAttack && _pmState.chainAtk > 0) {
                    _pmState.chainAtk--;
                    _pmState.act = 'attack';
                    _pmState.t = Date.now();
                    act = 'attack'; f = 0; _useW = false;
                } else {
                    _pmState.chainAtk = 0;
                    _pmState.act = null;
                    if (_wasSkill && _pmState.pendAtk && a.attack) { _pmState.pendAtk = false; _pmState.act = 'attack'; _pmState.t = Date.now(); act = 'attack'; f = 0; _useW = false; }   // 🔮 v3.0.94 技能播完→接播排隊中的攻擊動畫
...
if (typeof manualCast === 'function' && !manualCast._pmWrapped) {
    ...
    manualCast = function (skId) { return _pmOrigManualCast.apply(this, arguments); };   // skill 動畫改由 castSkill 成功後統一觸發（避免 MP 不足等失敗仍播、且與 castSkill 包裝重複）
...
function _allySpriteTriggerChainAttack(ally, n) {   // ⚔️ 傭兵多段物理攻擊技：連播 n 次攻擊動畫
    try {
        if (!(n > 0) || !ally || ally._slot == null) return;
...
function _allySpritesApply() {   // 8fps ticker 驅動（先於 _playerMorphApply→_partyBottoms 供主玩家消費）
    ...
    allies.forEach((ally, i) => {
        ...
        let st = _allySpriteStates[slot] || (_allySpriteStates[slot] = { act: null, t: 0, prevHp: null, el: null, imgs: null, key: null, skGen: false, pendAtk: false, chainAtk: 0 });
        if (st.key !== form.key) { if (st.el) { try { st.el.remove(); } catch (e) {} } st.el = null; st.imgs = null; st.act = null; st.key = form.key; st.pendAtk = false; st.chainAtk = 0; }   // 🔮 v3.0.96 換形態→清排隊攻擊
        // 受擊：curHp-delta（涵蓋所有傷害落點·不打斷施法/死亡）
        let hp = ally.curHp || 0;
        if (st.prevHp != null && hp < st.prevHp && hp > 0 && st.act !== 'death' && st.act !== 'skill') { st.act = 'hurt'; st.t = Date.now(); }
        st.prevHp = hp;
        // 倒地/復活
        ...
        if (st.act) {
            ...
            if (seq) {
                ...
                if (st.act === 'death') { act = 'death'; f = Math.min(ff, seq.length - 1); }
                    ...
                    let _wasAttack = (st.act === 'attack');
                    if (_wasAttack && st.chainAtk > 0) {
                        st.chainAtk--;
                        st.act = 'attack';
                        st.t = Date.now();
                        act = 'attack'; f = 0; _useW = false;
