...
// ===== 🪆 魔法娃娃：袋子（開袋）/ 金卡兌換 / 合成（即時 Math.random()，可 save/load 重抽）=====
...
function _dollRng(tag, seq) { return Math.random(); }   // 即時亂數（seq 保留相容舊呼叫，不參與計算）
...
// 🎁 高級魔法娃娃的盒子：80% 二階 / 18% 三階 / 2% 四階（總權重 10000）；選定階後該階娃娃「平均」抽一隻。
...
//   只有「放 4 個」失敗才累積 1 點保底（per-tier 分開）；保底達 5 時下次 4 個合成必定成功並清空。
...
//   100% 成功（2→1 已是代價，不再賭機率）；⚠️同 dollSynth 只讀寫背包、不碰倉庫。
