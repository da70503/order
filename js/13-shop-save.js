...
function saveGame() {
    ...
    player.config = {
        ...
        setAutoBuyMeat: document.getElementById('set-auto-buy-meat').checked,
...
function loadGame() {
    ...
    if (s) {
        ...
        // ⚠️ 還原 v2.6.40 升級表：曾遷移至天堂經典表（expMigV=2）的存檔，以進度 % 等比換算回舊表。
        if ((player.expMigV || 0) === 2) {
            let _mig3 = (lv, exp) => {
                lv = Math.max(1, Math.min(100, Math.floor(lv || 1)));
                if (lv >= 100) return 0;
                let c = _expReqClassic(lv), o = _expReqOldV1(lv);
                if (!isFinite(c) || !isFinite(o) || c <= 0) return 0;
                return Math.min(Math.floor(Math.max(0, exp || 0) / c * o), o - 1);
            };
        ...
        if (player.config) {
            ...
            if (c.setAutoBuyMeat !== undefined) document.getElementById('set-auto-buy-meat').checked = c.setAutoBuyMeat;
