...
function renderTabs(force) {
    ...
    let _scroll = {};
    ['tab-items','tab-weapons','tab-armors','tab-equip','tab-skill'].forEach(id => { let el = document.getElementById(id); if(el) { let sc=el.querySelector('.classic-skill-grid-scroll'); _scroll[id] = sc ? sc.scrollTop : el.scrollTop; } });   // 技能頁捲動在 .classic-skill-grid-scroll；其餘分頁直接記 tab 捲動
...
// 👇 一鍵強化到指定值：逐級嘗試直到目標值。安定值前必定成功；安定值起依固定機率表（與 doEnhance 同一套），
...
function executeCurseDeEnhance(targetUid, isEq, scrollId) {
    ...
    let d = DB.items[target.id];
    if (target.en <= -1) { logSys(`<span class="text-red-400 font-bold">${d.n} 已是 -1，無法再降低強化值。</span>`); return; }
