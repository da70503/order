...
function _autoBuyToTarget(itemId, target, chkId, qtyUnit) {   // 🍶 治療藥水同款：不足時一次補到目標數量（虛擬扣金幣·不開商店）
    try {
        ...
        // 箭／銀箭／肉：與 buyItem 同款捆綁價（1000 單位 = unit 金幣）·不可用 d.p（肉 d.p=1→攻城 8 折時 shopPrice(1)=0 會免費）
        let bundle = (itemId === 'wpn_5')        ? { unit: 100, amount: 1000 }
                   : (itemId === 'wpn_22')       ? { unit: 200, amount: 1000 }
                   : (itemId === 'new_item_143') ? { unit: 100, amount: 1000 }
                   : null;
        let totalCost = bundle
            ? Math.ceil(needed * shopPrice(bundle.unit) / bundle.amount)
            : needed * shopPrice(d.p);
        if (needed <= 0 || player.gold < totalCost) return false;
        player.gold -= totalCost;
        gainItem(itemId, needed, true, true);
        logSys(`自動消耗 ${totalCost} 金幣購買了 ${needed} ${qtyUnit || ''}${d.n}。`);
        return true;
...
//    屬性詞綴與遠古系詞綴不再由這些管道隨機產生（改由象牙塔『碧火』的賦予祝福卷軸取得）。
